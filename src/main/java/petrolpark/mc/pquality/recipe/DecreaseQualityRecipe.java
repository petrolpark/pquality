package petrolpark.mc.pquality.recipe;
...
import petrolpark.mc.recipe.contamination.IHandleContaminationMyself;
...
public class DecreaseQualityRecipe extends CustomRecipe implements IHandleContaminationMyself<CraftingContainer> {
    ...
    public ItemStack assemble(CraftingContainer container, RegistryAccess registryAccess) {
        return container.getItems().stream().dropWhile(ItemStack::isEmpty).findAny().map(stack -> {
            ...
            Flag flag = QualityUtil.getHighestQualityFlag(contamination);
            if (flag != null) contamination.decontaminateOnly(flag);
            return result;
